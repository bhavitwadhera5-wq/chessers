...
//#region node_modules/.nitro/vite/services/ssr/assets/humanBots-Dol-gB05.js
var HUMAN_BOTS = [
	{
		name: "Riley Carter",
		elo: 520,
...
if (HUMAN_BOTS.length !== 50) throw new Error(`Expected 50 bots, but found ${HUMAN_BOTS.length}.`);
if (new Set(HUMAN_BOTS.map((bot) => bot.name)).size !== HUMAN_BOTS.length) throw new Error("HUMAN_BOTS contains duplicate names.");
/**
* Pick a bot near the player's rating.
*/
function pickHumanBot(playerElo) {
	const target = playerElo + Math.round((Math.random() - .5) * 240);
	const pool = [...HUMAN_BOTS].sort((a, b) => Math.abs(a.elo - target) - Math.abs(b.elo - target)).slice(0, 5);
...
function humanBotByName(name) {
	return HUMAN_BOTS.find((bot) => bot.name === name) ?? null;
}
/**
...
function humanBotMove(fen, bot) {
	...
	if (Math.random() < bot.slip) {
		...
		const pool = captures.length > 0 && Math.random() > .45 ? captures : legal;
		return pool[Math.floor(Math.random() * pool.length)];
	}
	...
	const tolerance = Math.max(10, 150 - bot.elo / 24);
	const close = ranked.filter((item) => top - item.score * sign <= tolerance);
