...
import { i as playMoveSound, n as BoardView, o as setSoundEnabled, s as soundEnabled, t as BOARD_THEMES } from "./sounds-DLicoFiG.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/solo-7ikGglHG.js
...
function evaluate(chess) {
	...
	for (let r = 0; r < 8; r++) for (let f = 0; f < 8; f++) {
		...
		const v = VALUE$1[cell.type] + (PST[cell.type]?.[idx] ?? 0);
...
function scoreMove(m) {
	let s = 0;
	if (m.captured) s += 10 * VALUE$1[m.captured] - VALUE$1[m.piece];
...
function styleBonus(m, style) {
	const cap = m.captured ? VALUE$1[m.captured] : 0;
