...
async function applyRatings(gameId: string) {
  ...
  if (game.bot_name && (!game.white_id || !game.black_id)) {
    ...
    if (!profile) return;
    const score =
      game.result === "draw" ? 0.5 : game.result === (humanIsWhite ? "white" : "black") ? 1 : 0;
  ...
  const whiteScore = game.result === "white" ? 1 : game.result === "black" ? 0 : 0.5;
...
export const findOrCreateGame = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (data: { opponentUsername?: string | null; color?: "w" | "b" | "random"; minutes?: number }) =>
      data,
  )
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const wanted = (data.opponentUsername ?? "").trim().toLowerCase() || null;
    const minutes = [0, 5, 10].includes(Number(data.minutes)) ? Number(data.minutes) : 0;
    const pref = data.color ?? "w";
    const myColor: Color = pref === "random" ? (Math.random() < 0.5 ? "w" : "b") : pref;
    ...
    const { data: waiting } = await supabase
      .from("games")
      .select("id, white_id, black_id, invited_username, time_control")
      .eq("status", "waiting")
    ...
    const candidates = (waiting ?? []).filter(
      (g) =>
        ...
        (!g.invited_username || (myUsername && g.invited_username === myUsername)),
    );
    const preferred =
      candidates.find((g) => myUsername && g.invited_username === myUsername) ?? candidates[0];

    if (preferred) {
      const clockMs = minutes ? minutes * 60000 : null;
    ...
    return {
      gameId: created.id,
      mode: matchedName ? ("invited" as const) : ("open" as const),
...
export const makeMove = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { gameId: string; from: string; to: string }) => data)
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;

    const { data: game } = await supabase
      .from("games")
      .select("id, white_id, black_id, fen, status, time_control, white_ms, black_ms, turn_started_at")
      .eq("id", data.gameId)
      .maybeSingle();
    ...
    if (game.time_control > 0 && game.turn_started_at) {
      ...
      const remaining = (myColor === "w" ? whiteMs : blackMs) ?? game.time_control * 60000;
...
export const claimTimeout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const turn = new Chess(game.fen).turn() as Color;
    const remaining = (turn === "w" ? game.white_ms : game.black_ms) ?? game.time_control * 60000;
    const elapsed = game.turn_started_at
      ? Date.now() - new Date(game.turn_started_at).getTime()
      : 0;
    if (remaining - elapsed > 0) return { ok: false };
...
export const resignGame = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabase
      .from("games")
      .update({ status: "finished", result: myColor === "w" ? "black" : "white" })
      .eq("id", game.id);
    await applyRatings(game.id);
    return { ok: true };
  });

export const agreeDraw = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { gameId: string }) => data)
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
...
export const fillWithBot = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { gameId: string }) => data)
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { data: game } = await supabase
      .from("games")
      .select("id, white_id, black_id, status, time_control, bot_name")
      .eq("id", data.gameId)
      .maybeSingle();
    if (!game) throw new Error("Game not found.");
    if (game.white_id !== userId && game.black_id !== userId)
      throw new Error("You are not a player in this game.");
    if (game.status !== "waiting") return { filled: false, botName: game.bot_name };
    if (game.white_id && game.black_id) return { filled: false, botName: null };
    ...
    const { data: me } = await supabase
      ...
      .eq("id", userId)
      .maybeSingle();
    const bot = pickHumanBot(me?.elo ?? 1000);
    const clockMs = game.time_control ? game.time_control * 60000 : null;
    ...
    return { filled: true, botName: bot.name, botElo: bot.elo };
  });

/** Applies the computer opponent's move, requested by the human's client. */
export const botPlayMove = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { gameId: string; from: string; to: string }) => data)
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    ...
    if ((botColor === "w" ? game.white_id : game.black_id) !== null)
      throw new Error("That seat is taken by a player.");
    ...
    if (game.time_control > 0 && game.turn_started_at) {
      const elapsed = Date.now() - new Date(game.turn_started_at).getTime();
      const remaining = (botColor === "w" ? whiteMs : blackMs) ?? game.time_control * 60000;
      const left = remaining - elapsed;
      if (left <= 0) {
        await supabase
          .from("games")
          ...
          .eq("id", game.id);
        await applyRatings(game.id);
        return { fen: game.fen, status: "finished", result: botColor === "w" ? "black" : "white" };
      }
    ...
    if (over) await applyRatings(game.id);
    return { fen: chess.fen(), status: over ? "finished" : "active", result };
  });
